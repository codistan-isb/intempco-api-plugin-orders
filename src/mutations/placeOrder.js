...
export default async function placeOrder(context, input) {
  ...
  console.log("payment id is ", payments);

  const oId = order?._id;
  const paymentIds = payments.map((obj) => obj._id);

  let capturePayments = await context.mutations.captureOrderPayments(context, {
    orderId: oId,
    paymentIds,
    shopId,
  });

  console.log("Order captured ", capturePayments);
